// Package metrics provides functionality for collecting and reporting server metrics
package metrics
...
func (m *ServerMetrics) Report() string {
	...
	if received > 0 {
		avgLatency = float64(atomic.LoadInt64(&m.TotalLatency)) / float64(received) / 1e6 // Convert to ms
	}
...
func (m *ClientMetrics) Report() string {
	...
	sent := atomic.LoadInt64(&m.MessagesSent)
	received := atomic.LoadInt64(&m.MessagesReceived)
	latencySum := atomic.LoadInt64(&m.TotalLatency)
	errors := atomic.LoadInt64(&m.ErrorCount)
	...
	if received > 0 {
		avgLatency = float64(latencySum) / float64(received) / 1e6 // Convert to ms
	}
	...
	if sent >= received {
		packetLoss = fmt.Sprintf("%.2f%%", 100*(1-float64(received)/float64(sent)))
	} else {
		packetLoss = "N/A"
	}
